...
object Scan_SMSDRF {
  def Scan_SMSDRF_Graph(spark: SparkSession, out_source_sequence_wise_processing_draft_sms_data: DataFrame): DataFrame = {
    val variables = Map[String, Any]()
    import spark.implicits._  /* Source Sequence_wise_Processing.Scan_SMSDRF */
  lazy val t13 = out_source_sequence_wise_processing_draft_sms_data.select(substring(col("CMLS_CPD_DT"), 3, 2).as("BR_AUDT_YY"),
substring(col("CMLS_CPD_DT"), 5, 2).as("BR_AUDT_MM"),
substring(col("CMLS_CPD_DT"), 7, 2).as("BR_AUDT_DD"),
concat(col("CMLS_TRAN_CD"), when(col("CMLS_TRAN_CD") === lit("05") or col("CMLS_TRAN_CD") === lit("06") or col("CMLS_TRAN_CD") === lit("07"), when(col("CMLS_TRAN_DRCTN") === lit("SS"), when(col("CMLS_SMS_MSG_CLS") === lit(5) or col("CMLS_SMS_MSG_CLS") === lit(6) or col("CMLS_SMS_MSG_CLS") === lit(7), when(col("CMLS_POSCOND_CD") =!= lit("13"), lit("1")).otherwise(lit("2"))).otherwise(lit("2"))).otherwise(when(col("CMLS_USAGE_CD") === lit("1"), lit("1")).otherwise(lit("2")))).otherwise(lit(" ")), when(col("CMLS_SETLMT_SVC_ID") === lit(1), lit("I")).otherwise(lit("N")), when(col("CMLS_RTRN_RSN_FLG") =!= lit(" "), lit("R")).otherwise(lit(" ")), when(col("CMLS_RTRN_RSN_FLG") === lit(" "), when(col("CMLS_ACCT_USAGE_CD") === lit("C"), lit("C")).otherwise(when(col("CMLS_ACCT_USAGE_CD") === lit("D") or col("CMLS_ACCT_USAGE_CD") === lit("F"), lit("D")).otherwise(lit(" ")))).otherwise(lit(" ")), col("CMLS_COLL_FLG"), when(col("CMLS_TRAN_DRCTN") === lit("BB"), lit("B")).otherwise(when(col("CMLS_TRAN_DRCTN") === lit("SB"), lit("F")).otherwise(when(col("CMLS_TRAN_DRCTN") === lit("BS"), lit("R")).otherwise(lit("S"))))).as("BR_AUDT_TYPE1"),
...
col("*"))
  lazy val t14 = t13.select($"BR_AUDT_YY",
$"BR_AUDT_MM",
$"BR_AUDT_DD",
$"BR_AUDT_TYPE1",
$"BR_AUDT_TYPE2",
...
$"CMLS_SETLMT_CD",
$"BR_AUDT_FILLER")
  lazy val out_source_sequence_wise_processing_scan_smsdrf = t14
    out_source_sequence_wise_processing_scan_smsdrf
}
}
}
